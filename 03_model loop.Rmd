...
for(i in c(3, 4, 7, 8)) {
  ...
  summary(dat_10_long$cluster_10)
  ...
  results <- results |>
    mutate(p_value = case_when(results$mean < 0 ~ (1 - results$p_value),
                               results$mean > 0 ~ results$p_value))
  
  # results$p_value <- ifelse(results$mean < 0,
  #                           (1 - results$p_value),
  #                           results$p_value)
  # .. the strangest thing; ^this doesn't work
...
# multiple comparisons with false discovery rate
cl_results$p_fdr <- p.adjust(unlist(cl_results$p_value), method = "fdr")
cl_results$p_value <- cl_results$p_value$p_value
cl_results <- cl_results |>
  arrange(cluster, week)

# write.csv(cl_results, "data/results/inla_results.csv", row.names = FALSE)
# don't duplicate information (written below)

cl_results$sig <- ifelse(cl_results$p_fdr < 0.05, "significant", "not")

write.csv(cl_results, "data/results/cl_results.csv", row.names = FALSE)

# plot all results, by FDR significance
cols <- c("significant" = "red", "not" = "black")

library(extrafont)

cl_plt <- ggplot(cl_results, aes(x = week, y = or, colour = sig)) +
  geom_point() +
  geom_linerange(aes(ymin = lwr, ymax = upr)) +
  geom_hline(yintercept = 1) +
  scale_colour_manual(values = cols, labels = c("p < 0.05 after FDR", "p ≥ 0.05 after FDR")) +
...
cl_results |>
  ...
  ggplot(aes(x = week, y = or, colour = sig)) +
  geom_point() +
  geom_linerange(aes(ymin = lwr, ymax = upr)) +
  geom_hline(yintercept = 1) +
